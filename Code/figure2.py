...
from functions import elliptic_normal_matrix_opti, uniform_normal_matrix_opti


# %%
# Transition to feasibility in the case of Gaussian correlated distribution.
...
# Transition to feasibility in the case of Uniform distribution.
...
# Here, we choose the parameter a to have the right rho value.
a_index = np.array([-1.415, -1.02, -0.45])
NBR_RHO_IT = 3
...
for a in a_index:
    ...
    for kappa in ind:
        ...
        for i in range(MC_PREC):
            ...
            A = uniform_normal_matrix_opti(n, a)/np.sqrt(n)
            sol = np.dot(np.linalg.inv(
                np.eye(n, n)-(1/(kappa*np.sqrt(np.log(n))))*A), np.ones(n))  # Compute the solution
